...
public class HashTrieMap<A extends Comparable<A>, K extends BString<A>, V> extends TrieMap<A, K, V> {
    public class HashTrieNode extends TrieNode<Dictionary<A, HashTrieNode>, HashTrieNode> {
        ...
        public HashTrieNode(V value) {
            this.pointers = (Dictionary<A, HashTrieNode>) new ChainingHashTable<A, HashTrieNode>(AVLTree::new);
            this.value = value;
        }
        ...
        private class HashTrieNodeIterator extends SimpleIterator<Entry<A, HashTrieNode>> {
            ...
            public Entry<A, HashTrieNode> next() {
                Item<A, HashTrieNode> nextItem = pointers.iterator().next();
    ...
    public V insert(K key, V value) {
        ...
        for (A next : key) {
            if (curr.pointers.find(next) == null) { //check if we need to add new key
                curr.pointers.insert(next, new HashTrieNode());
            }
            curr = curr.pointers.find(next); //follow the path to insert
        }
    ...
    public V find(K key) {
        ...
        while(iter.hasNext()) { //if iter doesn't have next
            A next = iter.next();
            if(curr.pointers.find(next)!= null) {
                curr = curr.pointers.find(next);
            } else {
                return null;
            }
        }
...
//                            clear();
//                        } else {
//                            lastValid.pointers.delete(lastValidChar);
//                        }
//                    }
//                }
//
